# PRAMAAN — Rebuild & Assignment Plan (5 People)
...
This is the 5-person split (no Soumika). Timeline & playback goes to Roshan; Findings goes to Navneet.
...
|---|---|---|---|
| **Navneet** | Identification (REVAMP), Acquisition (FIX), Findings (TRIM) | `manufacturer_detect.py`, `device_structure.py`, `services/acquisition.py`, `services/disk_enumeration.py`, `services/e01_reader.py`, `services/ai_analytics.py`, `CaseAiAnalyticsPage.tsx` | Medium-heavy |
...
| **Roshan** | Recovery (page REVAMP), Timeline & playback (REVAMP) | `CaseRecoverPage.tsx`, `SegmentInspector.tsx`, `parsers/generic_tier2.py`, `parsers/filesystem_recovery.py`, `parsers/generic_fallback.py`, `services/timeline.py`, `parsers/temporal_sequencing.py`, `CaseTimelinePage.tsx`, `PlaybackDeck.tsx`, `TimelineView.tsx`, `FindingsTrack.tsx` | Heaviest slice — see note |
| **Jai Pranav** | Live devices (FIX), Report (FIX), Custody (KEEP), Overview (LIGHT), Cases (KEEP), Evidence catalog (LIGHT FIX), Job log (FIX), Settings (FIX) | `services/live_devices.py`, `api/v1/live.py`, `CaseLiveDevicesPage.tsx`, `services/logical_acquisition.py`, `services/reporting.py`, `CaseReportPage.tsx`, `CaseJobsPage.tsx`, `SettingsPage.tsx`, `CaseCustodyPage.tsx`, `CaseOverviewPage.tsx`, `CasesPage.tsx`, `CaseEvidenceCatalogPage.tsx`, Tauri shell + CI + the MVP demo script + the Section 1c purge | Medium + breadth + integration |

**Load note:** Roshan carries the most in this 5-person split — the Recovery page, the generic/E01 recovery path (currently crashing), *and* the whole Timeline menu with cross-camera normalisation. If it's too much mid-project, the two release valves are: (a) fold Findings into the Timeline track and drop menu 5 to cut scope, (b) hand the Recovery *page shell* (not the generic engine) to Aslam. The thing most at risk of a thin implementation is cross-camera normalisation — protect it, it's a PS requirement.

---

## 3. Per Person

### 3.1 NAVNEET — Identification + Acquisition + Findings

**Note for this area:** you own the first two things a judge touches, plus the (small) Findings menu. Identification and Acquisition both show fabricated numbers today. Aravind and Aslam need the device-info byte offsets from your identification research — pair with them early on where the model/serial string lives in each vendor's layout.
...
- Fix: make each source load independent (`Promise.allSettled`, not a single try/catch). Add `.dav` and `.mp4` to the file-input `accept`.
- Build: make the detected-disk list the primary imaging control — click a disk, it images. Typed path behind an "Advanced" toggle.
...
- Keep the "Include in report" toggle. If the team decides a separate menu isn't worth it, fold Findings into the Timeline "MOTION / AI FINDINGS" track and drop menu 5 — decide with Roshan and Jai Pranav.
- Done when: on the real `.dav`, findings ≤ 10 and every one is above the floor.

**Engine you own:** `manufacturer_detect.py`, `device_structure.py`, `services/acquisition.py`, `services/disk_enumeration.py`, `services/e01_reader.py`, `services/ai_analytics.py`, `CaseAiAnalyticsPage.tsx`.
...
- Output shape (Roshan consumes this): `channel, start_ts, end_ts, byte_offset, byte_length, event_type, resolution, fps, allocation_state`.
...
- Replace `confidence` with a passed-checks list. `docs/reference/dhfs.md` with sources. Use a real Dahua disk image if one can be obtained (second priority after Hikvision).
...
### 3.4 ROSHAN — Recovery menu (page) + generic/E01 recovery + Timeline & playback

**Note for this area:** you own the largest slice in the 5-person split — the Recovery page, the generic filesystem path (which currently crashes on a real E01), and the whole Timeline menu including cross-camera normalisation. Aslam owns the export bug beneath the playback deck; you own everything above it. If this is too much, cut scope by folding Findings into the Timeline track, or hand the Recovery page shell to Aslam.

**Generic / E01 recovery — the crash**
- Current bug (found in this audit): `generic_tier2` → `filesystem_recovery.py` (pytsk3) found **6 deleted files** in `nps-2013-canon1.E01`, then `recovery.py:_write_bounded_artifact` threw `ValueError: Invalid recovered byte range [2053899776, 2053900288) for 5982328-byte source` and the whole job **failed**. The filesystem reports offsets into the full logical volume; the artifact writer validates them against the E01 container file size. The two coincidental survivors are a 1-byte file and a 16 KB blob from parser "unknown".
- Fix: read recovered files through the same filesystem/EWF handle that found them (`tsk_file.read_random(...)`), not by seeking raw byte offsets into the E01 file.
...
- Fix: adapter chosen automatically from Identification; manual dropdown behind an "Advanced" toggle; dropdown updates when you change evidence.
- Build: a **Deleted** column, a header count ("4 recordings, 1 deleted"), a "deleted only" filter, a distinct row style — fed from the parsers' real `allocation_state` (Aravind / Aslam). Replace the confidence donut with a checks-passed breakdown. The SegmentInspector shows real metadata: channel, start/end, resolution, fps, codec, event type, both hashes.
...
- Current: pick a device; "Clock drift calibration" = one global offset per device; per-channel bar; "MOTION / AI FINDINGS" track; a multi-lane deck where each lane seeks independently and currently renders blank; `timeline.py` is a pure DB projection; `deleted_candidate` guessed from a validation-string set.
- Build:
  - a **2-channel test case** from existing real clips so you can work before the parsers deliver deleted flags;
  - a per-channel offset stored **per channel**, not one per device;
  - a **shared absolute time axis** in `timeline.py` + `TimelineView.tsx` when ≥2 channels have recorder timestamps;
  - per-lane offset nudge UI + auto-estimate from overlapping motion events;
  - one transport in `PlaybackDeck.tsx` — one scrub seeks **every** lane to the same absolute instant;
  - the deleted lane in a distinct colour with a legend and a "deleted only" toggle, fed from real `allocation_state`.
...
- Done when: the 2-channel case shows both lanes on one absolute axis, per-lane offset adjustable, one scrub moving both, deleted segments visually distinct.

**Engine you own:** `CaseRecoverPage.tsx`, `SegmentInspector.tsx`, `parsers/generic_tier2.py`, `parsers/filesystem_recovery.py`, `parsers/generic_fallback.py`, `services/timeline.py`, `parsers/temporal_sequencing.py`, `CaseTimelinePage.tsx`, `PlaybackDeck.tsx`, `TimelineView.tsx`, `FindingsTrack.tsx`.

---

### 3.5 JAI PRANAV — Live devices + Report + everything else + integration

**Note for this area:** one real feature (Live devices, currently broken by a one-character bug), seven menus that are mostly fine and need trimming, and the glue role — you run the fake-feature purge, own the weekly MVP demo, and keep custody/report rock-solid.
...
- Fix: build the URL properly (`&t=` or a `URL` object). Re-verify against a real RTSP source with G.711 audio that the grid shows live video, "Expand" plays with audio, Snapshot / Capture land in custody with correct hashes.
- Build: the grid tiles **all** connected devices' channels (`devices.flatMap(channels)`), not just the selected one. Wire the "Open pull panel" — channel `<select>` + start/end `datetime-local` → `logical_acquisition` bounds into ISAPI `CMSearchDescription` / Dahua `condition.*`. Exercise the ONVIF path.
...
- Move `test_live_devices_integration.py` into the default CI run (it's `-m integration` now); the mocked `test_live_devices.py` does not count as proof.
...
- Build: add a device block (make / model / firmware / serial / channel count from Navneet), a **deleted-recovery count** section, a per-recording metadata table (from Aravind / Aslam / Roshan). Confirm `report_state=INCLUDED` findings render in the PDF.

**Menu — Job log (FIX):** add a job-kind column (imaging / recovery / export / analytics / live-capture); show all kinds; failed jobs show the real error inline; kill the phantom blank-vendor recovery job at its source.
...
**Menu — Evidence catalog (LIGHT FIX):** remove the "Lab specimen" category, flask icon, facet, and the `filename.includes("specimen")` logic. Category from `acquisition_method` / `media_type`; status from `verification_status`; every facet option shows a real count. Add a recovered-recording count to the inspector.

**Menu — Custody / Overview / Cases:** Custody and Cases are KEEP — add a per-entry hash column to the custody table and a multi-device custody test. Overview: surface model/firmware when identification returns it; make "OEM identified %" reflect the strength word; kill the phantom recovery-job card.
...
- Write `scripts/demo/mvp.sh`: create case → acquire real image → identify (model shown) → recover (deleted segment found) → timeline (2 channels normalised) → custody log → signed report. It fails at first — that failure list is the team's backlog. Run it on every merge; block regressions.
- Own the Tauri desktop shell + CI config.

**Engine you own:** `services/live_devices.py`, `api/v1/live.py`, `CaseLiveDevicesPage.tsx`, `services/logical_acquisition.py`, `services/reporting.py`, `CaseReportPage.tsx`, `CaseJobsPage.tsx`, `SettingsPage.tsx`, `CaseCustodyPage.tsx`, `CaseOverviewPage.tsx`, `CasesPage.tsx`, `CaseEvidenceCatalogPage.tsx`, `scripts/dev/rtsp_test_source.py`, `scripts/demo/mvp.sh`.
...
| 4. Recover | Recovery | Roshan (page + generic) + Aravind / Aslam (vendor engines) | N recordings listed with real metadata; **at least one marked DELETED**; a "deleted only" filter isolates it. Job does not crash. |
| 5. Play a recovered recording | Recovery / Timeline | Aslam (pipeline) + Roshan (deck) | The recording — including the deleted one — exports to an MP4 that plays, with the right frame count. |
| 6. Normalised timeline | Timeline & playback | Roshan | ≥2 channels on one absolute axis; per-lane offset; one scrub moves all lanes; deleted lane distinct. |
