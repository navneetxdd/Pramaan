...
const caseWorkflow: NavItem[] = [
  ...
  { to: "device-id", label: "Identification", icon: Fingerprint, step: 2, requiresEvidence: true },

  { to: "recover", label: "Recovery", icon: ScanSearch, step: 3, requiresEvidence: true },

  { to: "timeline", label: "Timeline & playback", icon: GanttChartSquare, step: 4, requiresRecovery: true },

  { to: "ai-analytics", label: "Findings", icon: ScanEye, step: 5, requiresRecovery: true },
    ...
    if (locked) {
      ...
      return (
        ...
        <span
          ...
          title={item.requiresRecovery ? "Run recovery first" : "Acquire evidence first"}
    ...
    return (
      ...
      <NavLink key={item.label} to={dest} end={item.end}>
        ...
        {({ isActive }) => (
          ...
          <span className={cn("visily-nav-item", isActive && "visily-nav-item-active")}>
      ...
      {inCase ? (
        ...
        <div className="border-t p-3" style={{ borderColor: "var(--border-subtle)" }}>
          ...
          <NavLink to="/cases">
            ...
            <span className="visily-nav-item text-[11px] normal-case tracking-normal">← All cases</span>
...
export function useBreadcrumb(): { section: string; page: string } {
  ...
  if (pathname.startsWith("/settings")) return { section: "Pramaan", page: "Settings" };
