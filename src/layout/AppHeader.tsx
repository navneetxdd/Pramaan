...
import { ChevronDown, Lock, Search, Shield, Activity, Database } from "lucide-react";
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          {caseId && activeCase ? (
              ...
              >
                {cases.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name.slice(0, 32)} // {c.examiner_name}
                  </option>
                ))}
              </select>
            ...
            <Link to="/cases" className="mono text-[11px] font-semibold uppercase text-[var(--text-primary)] hover:text-[var(--accent-500)]">
              PRAMAN // Cases
            </Link>
          )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button type="button" className="btn-ghost h-8 w-8 p-0" title="Search (Ctrl+K)" onClick={openCommandPalette}>
