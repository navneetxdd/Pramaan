...
export function IntegrityPanel({ state, lastAudit, witnessHash, witnessLabel = "Chain tip hash", onVerify }: IntegrityPanelProps) {
      ...
      <div className="space-y-4 p-4">
        <div className="flex items-center justify-between rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-3)] px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] font-medium text-[var(--text-primary)]">Chain of custody</span>
          </div>
          <span className={secure ? "visily-badge visily-badge-success" : "visily-badge visily-badge-danger"}>
            {secure ? "Secure" : state === "checking" ? "Checking" : "Alert"}
          </span>
        </div>
        {lastAudit ? <p className="mono text-[11px] text-[var(--text-tertiary)]">Last audit: {lastAudit}</p> : null}
        {witnessHash ? (
        ...
        {onVerify ? (
          <Button variant="secondary" size="sm" className="w-full" onClick={onVerify}>
