      ...
      <div className="relative z-[1] flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="mono text-[10px] uppercase tracking-wider text-[var(--text-muted-on-dark)]">{since}</span>
          </div>
          <h1 className="text-[26px] font-semibold leading-tight tracking-tight text-[var(--text-on-dark)]">{title}</h1>
          <p className="max-w-xl text-[13px] leading-relaxed text-[var(--text-muted-on-dark)]">{description}</p>
            ...
            {meta.map((item) => (
              <div key={item.label}>
                ...
                <dd
                  ...
                  style={{ color: item.danger ? "#fca5a5" : "var(--text-on-dark)" }}
                >
                  {item.value}
                </dd>
              </div>
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          {secondaryAction?.to ? (
            ...
            <Button variant="secondary" className="border-white/20 bg-white/10 text-white hover:bg-white/15" onClick={secondaryAction.onClick}>
              {secondaryAction.label}
            </Button>
          ) : null}
          {primaryAction ? (
            <Button asChild className="bg-[var(--accent-500)] hover:bg-[var(--accent-400)]">
