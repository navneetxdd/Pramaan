...
export function ConfidenceDonut({
  ...
  const segmentTotal = total ?? high + medium + low;
  const displayTotal = segmentTotal > 0 ? segmentTotal : 0;
  const chartTotal = Math.max(displayTotal, 1);
  const radius = 42;
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      >
        ...
        <text
          ...
          y="48"
          textAnchor="middle"
          className="fill-[var(--text-primary)] text-[14px] font-semibold"
        >
          {displayTotal}
        </text>
        <text
          ...
          y="62"
          textAnchor="middle"
          className="fill-[var(--text-tertiary)] text-[8px]"
        >
          segments
        </text>
      </svg>
...
function logTone(line: string): string {
  if (
    /dual_signature|hkvi_block_4|honeywell_index_4|honeywell_format_carve_4/.test(
      line,
    )
  ) {
