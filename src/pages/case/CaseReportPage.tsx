...
export function CaseReportPage() {
  ...
  const hasBuilderImage = Boolean(
    workspace?.evidence.some((e) => e.acquisition_class),
  );
  const builderBanner =
    workspace?.evidence.find((e) => e.acquisition_class)?.acquisition_class
      ?.message ??
    "Builder image. Generated by this tool, not a field acquisition.";
  const jobCount =
    workspace?.jobs.filter(
      (j) => j.kind === "recovery" && j.status === "completed",
    ).length ?? 0;
        ...
        <section className="visily-card p-3">
          ...
          <ul className="space-y-1 text-[13px] text-[var(--text-secondary)]">
            ...
            <li className="flex items-center gap-2">
              ...
              Recovery {jobCount === 1 ? "run" : "runs"} ({jobCount})
            </li>
            <li className="flex items-center gap-2">
