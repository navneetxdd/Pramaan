...
export function CaseReportPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([api.report(caseId), api.version()])
      ...
      .catch((err) => {
        ...
        const message = err instanceof Error ? err.message : "Report load failed";
  ...
  const chainOk = Boolean((summary as { custody_chain_valid?: { ok?: boolean } } | null)?.custody_chain_valid?.ok);
        ...
        <div className="relative z-[1] flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12px] text-[var(--text-muted-on-dark)]">
              Live HTML preview of the signed deliverable judges read in court prep.
            </p>
            {fingerprint ? (
              <p className="mono mt-2 text-[10px] text-[var(--text-muted-on-dark)]">
                signature: {fingerprint.slice(0, 24)}… · self-signed integrity only
              </p>
            ) : null}
          </div>
      ...
      <div className="grid min-h-[560px] gap-3 lg:grid-cols-[240px_1fr]">
        <section className="visily-card p-3">
          ...
          <ul className="space-y-1 text-[13px] text-[var(--text-secondary)]">
            ...
            <li className="flex items-center gap-2">
              <Badge variant={chainOk ? "success" : "outline"}>{chainOk ? "✓" : "!"}</Badge>
              Custody chain {chainOk ? "intact" : "check required"}
            </li>
          </ul>
          {!loading && summary ? (
            <dl className="mt-4 space-y-1 border-t pt-3 text-[11px]" style={{ borderColor: "var(--border-subtle)" }}>
              <div className="flex justify-between gap-2">
                <dt className="text-[var(--text-tertiary)]">Segments</dt>
                <dd className="mono">{String((summary as { total_segments_recovered?: number }).total_segments_recovered ?? 0)}</dd>
              </div>
              <div className="flex justify-between gap-2">
                <dt className="text-[var(--text-tertiary)]">Generated</dt>
                <dd className="mono truncate">{String((summary as { generated_at?: string }).generated_at ?? "—")}</dd>
              </div>
            </dl>
          ) : null}
        </section>
        ...
        <section className="visily-card overflow-hidden">
          ...
          {loading ? (
            <p className="p-6 text-[13px] text-[var(--text-tertiary)]">Loading report preview…</p>
          ) : loadError ? (
            <p className="p-6 text-[13px] text-[var(--status-danger)]">{loadError}</p>
          ) : (
            <iframe
