...
export function CaseJobsPage() {
  ...
  const artifactTotal = jobs.reduce(
    (s, j) => s + (parseJobStats(j.stats_json).segmentsFound ?? 0),
    0,
  );
  ...
  function hashMetric(imageId: string, status: string) {
    if (status === "failed" || status === "error")
      return { label: "Job failed", tone: "danger" as const };
    const ev = evidence.find((e) => e.id === imageId);
    if (!ev?.sha256)
      return { label: "Pending verify", tone: "neutral" as const };
    if (status === "completed")
      return { label: "SHA-256 on file", tone: "success" as const };
    ...
    <div className="flex h-full min-h-0 flex-col gap-3">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button
              disabled={busy || evidence.length === 0 || recoveryRunning}
              onClick={() => void runRecoveryOnFirst()}
            >
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <DashboardStat
          ...
          value={formatBytes(parsedVolume)}
          icon={Database}
        />
        <DashboardStat
          label="Artifacts total"
          value={artifactTotal.toLocaleString()}
          icon={Layers}
        />
        <DashboardStat
          label="Failed jobs"
            ...
            {(["active", "completed", "all"] as Tab[]).map((t) => (
              <button
                ...
                className={`rounded px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${
                  tab === t
                    ? "bg-[var(--accent-soft)] text-[var(--accent-500)]"
                ...
                cell: (job) => {
                  ...
                  return (
                    <div>
                      ...
                      <span className="mono mt-0.5 block text-[10px] text-[var(--text-tertiary)]">
                        {progress.toFixed(0)}%
                      </span>
                    </div>
                  );
