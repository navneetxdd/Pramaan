...
import { HardDrive, Network, Play, RotateCcw, Server, Upload } from "lucide-react";
...
export function CaseAcquirePage() {
  ...
  const [oemImages, setOemImages] = useState<Array<{ filename: string; size_bytes: number }>>([]);
  ...
  const [logicalVendor, setLogicalVendor] = useState<"hikvision" | "dahua">("hikvision");
  ...
  async function handleAcquireOem(filename: string) {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "OEM acquire failed", { duration: Infinity });
  ...
  async function handleLogicalAcquire() {
    ...
    if (!actor.trim() || !logicalHost.trim() || !logicalUser.trim() || !logicalPassword) {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Logical acquisition failed", { duration: Infinity });
    ...
    <div className="flex h-full min-h-0 flex-col gap-3">
      ...
      <div className="visily-hero-dark px-5 py-4">
        ...
        <div className="relative z-[1]">
          ...
          <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--accent-400)]">Step 1 · Preservation</p>

          <h1 className="mt-1 text-[20px] font-semibold text-[var(--text-on-dark)]">Acquire evidence</h1>

          <p className="mt-1 max-w-2xl text-[12px] text-[var(--text-muted-on-dark)]">

            Image or register read-only copies. Source media is never modified — hashes are verified before the catalog.
            ...
            {STEPS.map((label, idx) => (
              ...
              <li
                ...
                className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wide ${
                  ...
                  idx <= activeStep ? "bg-[var(--accent-500)] text-white" : "border border-white/20 text-[var(--text-muted-on-dark)]"
      ...
      <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(260px,300px)_1fr_minmax(220px,260px)]">
        ...
        <section className="visily-card space-y-4 p-4">
          ...
          <div>
            ...
            <p className="visily-card-title text-[11px]">Who is performing this step?</p>

            <label className="mt-2 block text-[12px] font-medium text-[var(--text-primary)]">{HANDLER_FIELD_LABEL}</label>

            <Input className="mt-1" value={actor} onChange={(e) => setActor(e.target.value)} placeholder="e.g. SI Sharma" />

            <p className="mt-1 text-[11px] text-[var(--text-tertiary)]">{HANDLER_FIELD_HINT}</p>
          ...
          <div className="space-y-2 border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
            ...
            <p className="text-[11px] font-bold uppercase tracking-wide text-[var(--text-tertiary)]">Upload disk image</p>

            <Button variant="secondary" className="w-full justify-start" disabled={busy} onClick={() => void handlePickNative()}>
            ...
            <input ref={fileInputRef} type="file" accept=".bin,.img,.dd,.raw,.e01" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />

            {file ? <p className="mono truncate text-[11px] text-[var(--text-secondary)]">{file.name}</p> : null}

            <Button className="w-full" disabled={busy || !file} onClick={() => void handleAcquireUpload()}>
          ...
          <div className="space-y-2 border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
            ...
            <p className="text-[11px] font-bold uppercase tracking-wide text-[var(--text-tertiary)]">Block imaging</p>

            <Input placeholder="Path or \\.\PhysicalDriveN" value={filePath} onChange={(e) => setFilePath(e.target.value)} />
                ...
                {disks.slice(0, 3).map((disk) => (
                  ...
                  <button
                    ...
                    className={`flex w-full items-start gap-2 rounded border px-2 py-1.5 text-left text-[11px] ${
                      ...
                      selectedDisk?.id === disk.id ? "border-[var(--accent-500)] bg-[var(--accent-soft)]" : "border-[var(--border-subtle)]"
          ...
          <div className="space-y-2 border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
            ...
            <p className="text-[11px] font-bold uppercase tracking-wide text-[var(--text-tertiary)]">Operator drop folder</p>

            <p className="text-[11px] text-[var(--text-secondary)]">

              Drop E01, DD, or IMG files in <span className="mono">{oemDropLabel}</span>
            ...
            {oemImages.length === 0 ? (
              ...
              <p className="text-[11px] text-[var(--text-tertiary)]">No images found — run fetch script or copy media there.</p>
              ...
              <ul className="space-y-1">
                ...
                {oemImages.map((image) => (
                  ...
                  <li key={image.filename}>
                    ...
                    <Button variant="outline" size="sm" className="w-full justify-between" disabled={busy} onClick={() => void handleAcquireOem(image.filename)}>
                      ...
                      <span className="mono text-[10px]">{formatBytes(image.size_bytes)}</span>
          ...
          <details className="border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
            ...
            <div className="mt-2 space-y-2">
              ...
              <p className="text-[11px] text-amber-900">Clips only — no deleted/unallocated recovery.</p>

              <Input placeholder="NVR IP" value={logicalHost} onChange={(e) => setLogicalHost(e.target.value)} />

              <Input placeholder="Port" value={logicalPort} onChange={(e) => setLogicalPort(e.target.value)} />

              <Input placeholder="Username" value={logicalUser} onChange={(e) => setLogicalUser(e.target.value)} />

              <Input type="password" placeholder="Password (not stored)" value={logicalPassword} onChange={(e) => setLogicalPassword(e.target.value)} />

              <select className="field w-full" value={logicalVendor} onChange={(e) => setLogicalVendor(e.target.value as typeof logicalVendor)}>
              ...
              <Button className="w-full" variant="secondary" disabled={busy} onClick={() => void handleLogicalAcquire()}>
        ...
        <section className="visily-card flex flex-col p-5">
          ...
          <p className="visily-card-title mb-4 text-[11px]">Acquisition status</p>
          ...
          {busy ? (
            ...
            <div className="space-y-4">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-[var(--surface-3)]">
                ...
                <div className="h-full bg-[var(--accent-500)] transition-all duration-300" style={{ width: `${progress}%` }} />

              </div>

              <JobProgressCard title="Imaging in progress" subtitle={statusMessage || "Reading source…"} status="running" progress={progress} />
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-2)] p-4">
                ...
                <p className="text-[10px] font-bold uppercase tracking-wide text-[var(--text-tertiary)]">Registered evidence</p>

                <p className="mt-1 text-[16px] font-semibold text-[var(--text-primary)]">{latest.filename}</p>
              ...
              {hashState === "verified" ? (
                ...
                <Button asChild className="w-full sm:w-auto">
                  ...
                  <Link to={`/cases/${caseId}/device-id`}>Continue to identification →</Link>
            ...
            <div className="flex flex-1 flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-[15px] font-medium text-[var(--text-primary)]">No evidence registered</p>

              <p className="mt-2 max-w-sm text-[13px] text-[var(--text-secondary)]">

                Choose a source on the left — upload, image a path, or register a file from the operator drop folder.
        ...
        <aside className="visily-card flex min-h-0 flex-col">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2">
            ...
            {queueItems.length === 0 ? (
              ...
              <p className="p-3 text-[12px] text-[var(--text-tertiary)]">Evidence items appear here after acquisition.</p>
              ...
              <ul className="space-y-2">
                ...
                {queueItems.map((item) => (
                  ...
                  <li key={item.id} className="rounded border border-[var(--border-subtle)] p-3">

                    <p className="truncate text-[12px] font-medium">{item.label}</p>

                    <p className="mono mt-1 text-[10px] text-[var(--text-tertiary)]">{shortHash(item.id)}</p>

                    <div className="mt-2 flex items-center justify-between">

                      <span className="rounded bg-emerald-50 px-2 py-0.5 text-[9px] font-bold uppercase text-emerald-800">{item.status}</span>
