    ...
    <div className="flex h-full min-h-0 flex-col gap-3">
      <div className="visily-hero-dark px-5 py-4">
        ...
        <div className="relative z-[1] flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--accent-400)]">Step 2 · Identification</p>
            <h1 className="mt-1 text-[20px] font-semibold text-[var(--text-on-dark)]">Device & format analysis</h1>
            <p className="mt-1 max-w-xl text-[12px] text-[var(--text-muted-on-dark)]">
              Signature scan and partition layout — selects the recovery adapter. Routing hints only until field-validated.
            </p>
          </div>
      ...
      {evidence.length === 0 ? (
        ...
        <div className="grid min-h-[420px] gap-3 lg:grid-cols-[240px_1fr]">
          <section className="visily-card flex flex-col p-3">
            <p className="visily-card-title mb-2 text-[11px]">Evidence in this case</p>
                ...
                <li key={e.id}>
                  <button
                    ...
                    className={`w-full rounded-lg px-2 py-2 text-left ${
                      deviceId === e.id ? "bg-[var(--accent-soft)] ring-1 ring-[var(--accent-500)]/30" : "hover:bg-[var(--surface-3)]"
                    }`}
                    onClick={() => {
                      setDeviceId(e.id);
                  ...
                  >
                    <p className="truncate text-[12px] font-medium">{e.filename}</p>
                    <p className="mono mt-0.5 text-[10px] text-[var(--text-tertiary)]">{formatBytes(e.size_bytes)}</p>
                  </button>
                </li>
              ))}
            </ul>
            ...
            {flatStructure.length > 0 ? (
              <>
                <p className="visily-card-title mb-2 mt-4 text-[11px]">Layout tree</p>
                  ...
                  {flatStructure.map(({ node, depth }) => (
                    <button
                      ...
                      className={`block w-full truncate rounded px-1 py-0.5 text-left text-[11px] ${
                        selectedNode?.offset === node.offset && selectedNode?.label === node.label
          ...
          <div className="flex min-h-0 flex-col gap-3">
            <section className="visily-card p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="visily-card-title text-[11px]">Identification result</p>
                  {selectedEvidence ? (
                    <p className="mt-1 text-[14px] font-semibold text-[var(--text-primary)]">{selectedEvidence.filename}</p>
                  ) : null}
                </div>
              ...
              {!report ? (
                <p className="mt-4 text-[13px] text-[var(--text-tertiary)]">Run identification to see vendor hits and adapter routing.</p>
              ) : (
                <ul className="mt-4 space-y-2">
                  {report.hits.map((hit) => (
                    <li key={`${hit.vendor}-${hit.adapter}`} className="rounded-lg border border-[var(--border-subtle)] px-3 py-2">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          <p className="text-[13px] font-semibold text-[var(--text-primary)]">{hit.vendor}</p>
                          <p className="font-mono text-[11px] text-[var(--text-tertiary)]">{hit.adapter}</p>
                        </div>
                        <ConfidenceBadge tier={hit.confidence >= 0.8 ? "high" : hit.confidence >= 0.5 ? "medium" : "low"} />
                      </div>
                      {hit.markers.length > 0 ? (
                        <p className="mt-1 text-[11px] text-[var(--text-secondary)]">Markers: {hit.markers.join(", ")}</p>
                      ) : null}
                      {hit.capability_tier ? (
              ...
              {report && topHit ? (
                <Button asChild className="mt-4" variant="secondary">
                  <Link to={`/cases/${caseId}/recover`}>Continue to recovery →</Link>
                </Button>
              ) : null}
            </section>
            ...
            <section className="visily-card flex min-h-0 flex-1 flex-col p-4">
              <div className="mb-2 flex items-center justify-between gap-2">
                <p className="visily-card-title text-[11px]">Hex at {formatOffset(hexOffset)}</p>
              ...
              {report?.filesystem_hints && report.filesystem_hints.length > 0 ? (
                <div className="mt-3 border-t pt-3" style={{ borderColor: "var(--border-subtle)" }}>
                  <p className="text-[10px] font-bold uppercase text-[var(--text-tertiary)]">Filesystem hints</p>
              ...
              {report?.coverage_note ? (
                <p className="mt-2 text-[11px] text-[var(--text-tertiary)]">{report.coverage_note}</p>
              ) : null}
            </section>
          </div>
        </div>
