...
export function CaseOverviewPage() {
  ...
  const [liveJobState, setLiveJobState] = useState<Record<string, { progress: number; message: string }>>({});
  ...
  const identified = evidence.filter((e) => e.identification?.recommended_adapter).length;
  const coveragePct = evidence.length === 0 ? "—" : `${Math.round((identified / evidence.length) * 100)}%`;
  const sortedJobs = [...jobs].sort((a, b) => (b.started_at ?? "").localeCompare(a.started_at ?? ""));
  ...
  return (
    <div className="mx-auto flex max-w-[1440px] flex-col gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <DashboardStat label="Evidence items" value={String(evidence.length)} icon={HardDrive} tone="info" />
        <DashboardStat label="OEM identified" value={coveragePct} hint={`${identified} of ${evidence.length} imaged`} icon={Shield} tone="success" />
        <DashboardStat label="Recovered segments" value={segmentTotal.toLocaleString()} icon={ScanSearch} />
        <DashboardStat label="Failed jobs" value={String(flagged).padStart(2, "0")} icon={Shield} tone={flagged > 0 ? "danger" : "success"} />
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="visily-card">
          <div className="visily-card-header">
            ...
            <Link to="recover" className="text-[11px] font-semibold uppercase tracking-wide text-[var(--accent-500)]">
              Manage
            </Link>
          </div>
          <div className="space-y-3 p-4">
            ...
            {jobs.length === 0 ? (
              <JobProgressCard title="No recovery jobs" subtitle="Acquire evidence on the Acquisition screen, then run recovery." status="idle" />
            ) : null}
          </div>
        </section>
        ...
        <div className="space-y-4">
          <IntegrityPanel
            state={custody}
            lastAudit={events.length > 0 ? events[events.length - 1].created_at.replace("T", " ").slice(0, 19) : "No events yet"}
      ...
      <section className="visily-card">
        <div className="visily-card-header">
          ...
          <Link to="evidence" className="text-[11px] font-semibold uppercase tracking-wide text-[var(--accent-500)]">
            Catalog
          </Link>
        </div>
        {evidence.length === 0 ? (
            ...
            {evidence.slice(0, 4).map((e) => (
              <EvidenceTile
                ...
                status={integrityLabel(
                  resolveIntegrityState(e.acquisition_status, e.verification_status),
                )}
        ...
        {evidence.length > 0 ? (
          <div className="border-t px-4 py-2" style={{ borderColor: "var(--border-subtle)" }}>
            <p className="mono text-[10px] text-[var(--text-tertiary)]">
              {formatBytes(totalBytes)} total · {evidence.length} item{evidence.length === 1 ? "" : "s"}
            </p>
          </div>
        ) : null}
      </section>
