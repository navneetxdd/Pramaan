"""Hikvision builder image. Generated bytes, not a field acquisition.

Used by the internal acquisition helper and the parser tests. It is kept alive
only so that helper keeps working; the parser is validated for real against the
emulated image in ``engine/tests/support/hikvision_builder.py``.
...
SPECIMEN_LABEL = b"PRAMAAN-BUILDER-IMG-HIKVISION"

_BUILDER_EPOCH = 1_700_000_000
...
def build_hikvision_builder_specimen() -> bytes:
    ...
    struct.pack_into("<I", master, MASTER_INIT_TIME_OFF, _BUILDER_EPOCH)
    ...
    for slot, (channel, block_index, allocated, overwritten_by) in enumerate(_LAYOUT):
        ...
        start = _BUILDER_EPOCH + 3600 + slot * 600
        end = start + 300
...
def write_hikvision_specimen(dest: Path) -> Path:
    ...
    dest.write_bytes(build_hikvision_builder_specimen())
    return dest
