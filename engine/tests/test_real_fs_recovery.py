...
class RealFilesystemRecoveryTests(unittest.TestCase):
    def test_fat16_deleted_inode_recovery(self) -> None:
        ...
        segments = GenericTier2Adapter().scan(FAT16_FIXTURE)
        deleted = [segment for segment in segments if segment.validation == "filesystem_deleted_inode"]
        self.assertGreaterEqual(len(deleted), 1, segments)
        self.assertTrue(any(segment.raw_bytes for segment in deleted))
    ...
    def test_public_canon2_oem_image_present(self) -> None:
        ...
        if not candidate.is_file():
            self.skipTest("Public canon2 E01 not fetched — run: python scripts/fetch_validation_assets.py --real-fs")
